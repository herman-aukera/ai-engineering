...
# Explicitly classified LOCAL_DEVELOPMENT_ONLY images. New mutable refs are not
# implicitly trusted; they must either become digest-pinned or be reviewed and
# added here with an exact path/ref pair.
...
def _check_image_value(path: Path, root: Path, lineno: int, value: str) -> str | None:
    ...
    if not value:
        return None
    if "${" in value:
        # Runtime-injected production image identities are accepted only when
        # the declaration itself requires an immutable digest.
        lowered = value.lower()
...
def find_mutable_image_refs(root: Path) -> list[str]:
    ...
    dockerfiles.extend(sorted(product_root.glob("Dockerfile*")))
    ...
    for path in dockerfiles:
        ...
        for lineno, line in enumerate(path.read_text(encoding="utf-8").splitlines(), 1):
            ...
            if image not in stages and _is_external_image(image) and not DIGEST.search(image):
                errors.append(
                    f"{path.relative_to(root)}:{lineno}: mutable Dockerfile base image: {image}"
                )
            if alias:
                stages.add(alias)
    ...
    if deploy.exists():
        yaml_paths.extend(sorted(p for p in deploy.rglob("*.yml") if p.is_file()))
        yaml_paths.extend(sorted(p for p in deploy.rglob("*.yaml") if p.is_file()))
