...
REQUIRED_INVARIANTS = {
    "deterministic_hard_constraints", "deterministic_budget_authority",
    "planned_served_distinct", "repair_creates_reevaluated_revision",
    "signed_human_authority", "replay_safe", "durable_authoritative_state",
    "secrets_excluded_from_evidence", "hidden_reasoning_excluded",
}
...
def _resolve(value: object) -> Path:
    ...
    for path in (PROJECT_ROOT / value, REPO_ROOT / value):
        if path.is_file():
            return path
...
def verify() -> dict[str, object]:
    ...
    if active & omitted or active | omitted != STAGES:
        raise AssertionError("Energy-Aware stages must be exclusively and completely accounted for")
    ...
    if failed:
        raise AssertionError(f"required authority invariants are not proven: {failed}")
    reason_codes = manifest.get("reason_codes")
    if not isinstance(reason_codes, list) or not reason_codes:
        raise AssertionError("reason_codes must be a non-empty list")
    invalid = sorted(code for code in reason_codes if not isinstance(code, str) or not REASON_CODE.fullmatch(code))
    if invalid:
        raise AssertionError(f"invalid stable reason codes: {invalid}")
    for key in ("production_entrypoint", "public_surface_owner", "dependency_lock", "dependency_lock_digest"):
        _resolve(manifest.get(key))
    ...
    source = _resolve(manifest.get("production_entrypoint")).read_text(encoding="utf-8") + "\n" + _resolve(manifest.get("public_surface_owner")).read_text(encoding="utf-8")
    markers = manifest.get("public_surface_markers")
    if not isinstance(markers, list) or not markers:
        raise AssertionError("public_surface_markers must be a non-empty list")
    missing_markers = [marker for marker in markers if not isinstance(marker, str) or marker not in source]
    if missing_markers:
    ...
    return {
        "schema_version": manifest["schema_version"], "protocol_version": version,
        "product": manifest["product"], "canonical_branch": manifest["canonical_branch"],
        "active_stage_count": len(active), "not_applicable_stage_count": len(omitted),
        "invariant_count": len(REQUIRED_INVARIANTS), "reason_code_count": len(reason_codes),
        "status": "pass",
    }
