...
def test_image_pin_validator_accepts_digest_pinned_runtime_images(tmp_path: Path) -> None:
    _write(tmp_path / "estimador-cag/Dockerfile", f"FROM python:3.11-slim@{IMAGE_DIGEST} AS runtime\nFROM runtime AS final\n")
    _write(tmp_path / ".github/workflows/ci.yml", f"services:\n  postgres:\n    image: postgres:16@{IMAGE_DIGEST}\n")
...
def test_image_pin_validator_rejects_mutable_ci_and_base_images(tmp_path: Path) -> None:
    ...
    _write(tmp_path / ".github/workflows/ci.yml", "services:\n  postgres:\n    image: postgres:16\n")
