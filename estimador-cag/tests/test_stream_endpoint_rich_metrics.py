...
class FakeCache:
    ...
    def make_key(self, *, tier, model, system_prompt, transcription):
        return f"stream-rich:{tier}:{model}:{transcription}"
...
def test_stream_endpoint_records_rich_metrics_on_cache_miss(monkeypatch):
    ...
    response = client.post(
        ...
        json={
            "transcription": "Unique rich stream miss transcript.",
            "tier": "flash",
        },
    )
    ...
    assert metrics["stream_output_chars"] == len("Hello stream")
    assert metrics["stream_cached"] is False
    assert metrics["stream_started_at"]
    assert metrics["stream_finished_at"]


def test_stream_endpoint_records_rich_metrics_on_cache_hit(monkeypatch):
    fake_cache = FakeCache()
    ...
    client = TestClient(app)
    payload = {
        "transcription": "Unique rich stream hit transcript.",
        "tier": "flash",
    }
    ...
    assert metrics["stream_output_chars"] == len("Hello stream")
    assert metrics["stream_cached"] is True
    assert metrics["stream_started_at"]
    assert metrics["stream_finished_at"]
