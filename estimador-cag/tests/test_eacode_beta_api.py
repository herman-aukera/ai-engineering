...
def test_beta_demo_requires_server_session_and_persists_completed_timeline(
    ...
    prepared_response = client.post("/eacode/demo", json=_payload("api-demo-1"))
...
def test_viewer_cannot_issue_or_consume_authorization(tmp_path, monkeypatch) -> None:
    monkeypatch.setenv("EACODE_DEMO_DB_PATH", str(tmp_path / "eacode.sqlite3"))
    monkeypatch.setenv("EACODE_SESSION_SIGNING_KEY", SIGNING_KEY)
    assert client.post("/eacode/demo", json=_payload("api-demo-2")).status_code == 201

    headers = {"Authorization": f"Bearer {_token('viewer', user_id='viewer-1')}"}
    assert (
        client.post("/eacode/demo/api-demo-2/authorize", headers=headers).status_code
        == 403
    )
...
def test_client_controlled_human_authorization_is_rejected(tmp_path, monkeypatch) -> None:
    ...
    response = client.post("/eacode/demo", json=payload)

    assert response.status_code == 422


def test_missing_demo_run_returns_404(tmp_path, monkeypatch) -> None:
    monkeypatch.setenv("EACODE_DEMO_DB_PATH", str(tmp_path / "eacode.sqlite3"))
