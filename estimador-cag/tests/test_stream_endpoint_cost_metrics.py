...
class FakeCache:
    ...
    def make_key(self, *, tier, model, system_prompt, transcription):
        return f"stream-cost:{tier}:{model}:{transcription}"
...
def test_stream_endpoint_records_cost_metadata_on_cache_miss(monkeypatch):
    ...
    response = client.post(
        ...
        json={
            "transcription": "Unique stream cost miss transcript.",
            "tier": "flash",
        },
    )
    ...
    assert metrics["cost_source"] == "missing_token_usage"
...
def test_stream_endpoint_records_cost_metadata_on_cache_hit(monkeypatch):
    ...
    client = TestClient(app)
    payload = {
        "transcription": "Unique stream cost hit transcript.",
        "tier": "flash",
    }
