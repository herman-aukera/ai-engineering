"""Small provider-neutral signed-session contract for EACHAT production identity."""
...
class ActorContext:
    """Authenticated actor identity used for ownership and audit decisions."""
...
class SignedSessionCodec:
    """HMAC-SHA256 backend session tokens with explicit expiry and tenant identity."""
    ...
    def issue(
        ...
        *,
        ...
        role_values = tuple(dict.fromkeys(_identity(role, "role") for role in roles))
        if not role_values:
        ...
        return f"{self.version}.{encoded_payload}.{_b64_encode(signature)}"
    ...
    def verify(self, token: str, *, now: datetime | None = None) -> ActorContext:
        ...
        payload_bytes = _b64_decode(encoded_payload, "payload")
