...
| 13. PostgreSQL | implemented | PostgresCheckpointer with LangGraph PostgresSaver, versioned schema DDL, CheckpointRetentionPolicy; 8 interface tests + 7 live-DB integration tests (skipped in CI, run with EACHAT_POSTGRES_URL); covers connection, schema migration, write/read, thread isolation, restart simulation, resume, retention, redaction |
